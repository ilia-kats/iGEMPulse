...
def concate(abstracts, results, tracks):
    ...
    for itemAbs in dataAbs:
        ...
        for itemRes in dataRes:
            if itemAbs['name'] == itemRes['name'] and itemAbs['year'] == itemRes['year'] and doubletest:
                newData = {}
                print i
                newData['name'] = itemAbs['name'][0]
                newData["year"] = int(itemAbs['year'][0])
                newData["title"] = (itemAbs['title'][0] if itemAbs['title'] else "")
                newData["link"] = (itemAbs['link'][0] if itemAbs['link'] else "no medal")
...
test = concate("abstracts.json", "results.json", "tracks.json")
#print test
filename = "whole_dataset.json"
